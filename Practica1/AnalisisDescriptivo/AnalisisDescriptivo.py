...
def EjecutarAnalisisDescriptivo():
    ...
    with open("Resultados.md", "w") as Archivo:
        ...
        Archivo.write("A continuacion se presentan las graficas para el analisis exploratorio:\n\n")
        Archivo.write("### 1. Ventas por Mes\n")
        Archivo.write("![Ventas por Mes](ventas_por_mes.png)\n")
        Archivo.write("*Explicacion*: Se observa la distribucion del monto total de compras agrupadas por mes, permitiendo identificar claramente los picos de ventas durante el año.\n\n")
        Archivo.write("### 2. Ventas por Metodo de Pago\n")
        Archivo.write("![Ventas por Metodo de Pago](ventas_por_metodo_pago.png)\n")
        Archivo.write("*Explicacion*: Compara el volumen de ingresos segun el metodo de pago utilizado (0: Efectivo, 1: Tarjeta de Credito, 2: Tarjeta de Debito).\n\n")
        Archivo.write("### 3. Ventas por Navegador\n")
        Archivo.write("![Ventas por Navegador](ventas_por_navegador.png)\n")
        Archivo.write("*Explicacion*: Muestra el comportamiento de compra segmentado por el navegador que utilizo el cliente.\n\n")
        Archivo.write("### 4. Ventas con Boletin\n")
        Archivo.write("![Ventas por Boletin](ventas_por_boletin.png)\n")
        Archivo.write("*Explicacion*: Visualiza el monto de ventas comparando las transacciones que utilizaron un boletin versus las que no.\n\n")
        Archivo.write("### 5. Ventas con Vale\n")
        Archivo.write("![Ventas por Vale](ventas_por_vale.png)\n")
        Archivo.write("*Explicacion*: Contrasta las ventas donde se aplico un vale promocional contra las que no.\n\n")
        
        Archivo.write("## Explicacion Paso a Paso del Script de Analisis (`AnalisisDescriptivo.py`)\n")
        Archivo.write("El script fue disenado para automatizar la extraccion y calculo de estadisticas de la base de datos de AWS. A continuacion se detalla su funcionamiento paso a paso:\n\n")
        Archivo.write("### 1. Conexion a la Base de Datos\n")
        Archivo.write("Se utiliza la libreria `dotenv` para leer las credenciales seguras y `sqlalchemy` para conectarse a PostgreSQL.\n")
        Archivo.write("```python\n")
        Archivo.write("def ObtenerConexionDb():\n")
        Archivo.write("    load_dotenv(dotenv_path='../.env')\n")
        Archivo.write("    Url = f\"postgresql+psycopg2://{User}:{Password}@{Host}:{Port}/{Name}?sslmode={SslMode}\"\n")
        Archivo.write("    return create_engine(Url)\n")
        Archivo.write("```\n\n")
        Archivo.write("### 2. Extraccion y Union de Datos (ETL en memoria)\n")
        Archivo.write("Se extraen las tablas principales mediante consultas SQL directas y se unen (`merge`) usando Pandas para obtener un dataset completo por cliente.\n")
        Archivo.write("```python\n")
        Archivo.write("DfClientes = pd.read_sql(\"SELECT * FROM ventas.clientes;\", Engine)\n")
        Archivo.write("DfCompras = pd.read_sql(\"SELECT * FROM ventas.compras;\", Engine)\n")
        Archivo.write("DfCompleto = pd.merge(DfCompras, DfClientes, on=\"id_cliente\", how=\"inner\")\n")
        Archivo.write("```\n\n")
        Archivo.write("### 3. Calculo de Estadisticas y Segmentacion\n")
        Archivo.write("Aprovechando las funciones de agregacion de Pandas (`.mean()`, `.median()`, `.mode()`), calculamos las estadisticas basicas y utilizamos `.groupby()` para segmentar.\n")
        Archivo.write("```python\n")
        Archivo.write("VentasPorMes = DfCompras.groupby('mes_compra')['monto_compra'].sum().reset_index()\n")
        Archivo.write("ComprasPorEdad = DfCompleto.groupby('grupo_edad')['monto_compra'].sum().reset_index()\n")
        Archivo.write("```\n\n")
        Archivo.write("### 4. Generacion de Graficos (Visualizacion)\n")
        Archivo.write("Finalmente, se emplean `seaborn` y `matplotlib` para renderizar los graficos y exportarlos como imagenes `.png` listas para el informe.\n")
        Archivo.write("```python\n")
        Archivo.write("sns.barplot(data=VentasPorMes, x='mes_compra', y='monto_compra')\n")
        Archivo.write("plt.savefig('ventas_por_mes.png')\n")
        Archivo.write("```\n\n")
